                        ...
                        <table class="table table-striped table-bordered table-hover table-checkable order-column" id="sample_1">
                            ...
                            <tbody>
                                <?php foreach ($data as $row) { ?>
                                    <tr>
                                        <?php $this->db->where('ID_nawyat_maktob',$row->ID);
                                              $this->db->from('makateb');
                                              $mak=$this->db->get()->result();
                                              foreach($mak as $value){
                                         ?>
                                        <td style="text-align: center;"><?=$value->shumara_maktob?></td>
                                        <td style="text-align: center;"><?=$value->mursal?></td>
                                        <td style="text-align: center;"><?=$row->nawyat?></td>
                                        <td style="text-align: center;"><?=$value->khulas_matlab?></td>
                                        <td style="text-align: center;"><?=$value->ejraat?></td>
                                    <?php }?>
                                        <td style="text-align: center;">
                                         <a href="<?=base_url()?>index.php/sadraController/sadraDetail?id=<?=$row->ID?>" class="btn btn-circle btn-icon-only  green-jungle"><span class="glyphicon glyphicon-eye-open "></a>
                                        <a class="btn btn-circle btn-icon-only btn-success" href="<?=base_url()?>index.php/araizController/editaraiz?id=<?=$row->ID?>"><span class="fa fa-pencil"></a>
